...
/datum/component/dexterity/Attach(datum/target, storage_type = ITEM_SLOT_DEX_STORAGE, hud_type = /datum/hud/dextrous, has_internal_storage = FALSE)
	...
	ADD_TRAIT(target, TRAIT_ADVANCEDTOOLUSER, REF(src))
	ADD_TRAIT(target, TRAIT_CAN_STRIP, REF(src))
...
/datum/component/dexterity/proc/on_examine(datum/source, mob/user, list/examine_list)
	...
	if(internal_storage && !(internal_storage.item_flags & ABSTRACT))
		examine_list += "It is holding [internal_storage.get_examine_string(user)] in its internal storage."
...
/datum/component/dexterity/proc/drop_items(mob/living/user)
	SIGNAL_HANDLER
...
/datum/component/dexterity/proc/equip_to_internal_storage(mob/living/user, obj/item/thing, slot)

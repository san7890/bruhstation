/proc/assemble_map_selections()
	RETURN_TYPE(/list)
	var/static/list/map_rotate_choices = null
	...
	for(var/map in config.maplist)
		...
		map_rotate_choices[mapname] = virtual_map
...
ADMIN_VERB(admin_change_map, R_SERVER, "Change Map", "Set the next map.", ADMIN_CATEGORY_SERVER)
	...
	var/chosenmap = tgui_input_list(user, "Choose a map to change to", "Change Map", sort_list(map_rotate_choices)|"Custom")
	if(isnull(chosenmap))
		return

...
/mob/living/basic/guardian/assassin
	...
	/// The action that actually lets us do the stealth ability.
...
/mob/living/basic/guardian/assassin/get_status_tab_items()
	. = ..()
	if(stealth_action.next_use_time > world.time)
		. += "Stealth Cooldown Remaining: [DisplayTimeText(stealth_action.next_use_time - world.time)]"

/mob/living/basic/guardian/assassin/adjust_health(amount, updating_health = TRUE, forced = FALSE)
	. = ..()
	if(. > 0 && toggle)
		visible_message(span_danger("\The [action_holder] suddenly appears!"))
		ToggleMode()

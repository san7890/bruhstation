...
/mob/living/create_thinking_indicator()
	if(active_thinking_indicator || active_typing_indicator || !thinking_IC || stat != CONSCIOUS )
		return FALSE
	// If we have this trait (generated by a custom station trait), pass in a random bubble type every time we speak.
	if(HAS_TRAIT(SSstation, STATION_TRAIT_JUMBLED_BUBBLES))
		var/datum/speech_bubble/random_bubble = pick(typesof(/datum/speech_bubble)) // Pick a random bubble datum
		bubble_icon = new random_bubble
	var/file = bubble_icon.icon_file
	active_thinking_indicator = mutable_appearance(file, "[bubble_icon.icon_state]3", TYPING_LAYER, src, SOUND_EFFECT_VISUAL_PLANE)
...
/mob/living/create_typing_indicator()
	...
	if(HAS_TRAIT(SSstation, STATION_TRAIT_JUMBLED_BUBBLES))
		...
		bubble_icon = new random_bubble
	var/file = bubble_icon.icon_file
	active_typing_indicator = mutable_appearance(file, "[bubble_icon.icon_state]0", TYPING_LAYER, src, SOUND_EFFECT_VISUAL_PLANE)
